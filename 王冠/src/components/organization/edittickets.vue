    ...
    <div class="organization_personinfo_datailwrapper">
      <Form :model="activity" label-position="right" :label-width="100">
        ...
        <Form-item label="抢票截止时间">
          <Row>
            <Col span="11">
            <Date-picker type="datetime" format="yyyy-MM-dd HH:mm" placeholder="选择日期和时间" v-model="activity.cutofftime" disabled></Date-picker>
            </Col>
          </Row>
        </Form-item>
      </Form>
  ...
  <div class="organization_personinfo_card">
    ...
    <div class="organization_personinfo_datailwrapper">
      <Form ref="activity" :model="activity" :label-width="80">
        ...
        <Row>
          <Col span="12">
            <Input type="text" v-model="item.value" placeholder="请输入票的类型" disabled></Input>
          </Col>
          <Col span="4" offset="1" >
             <Input v-model="item.num" placeholder="票数" disabled></Input>
          </Col>
          <Col span="4" offset="2">
            <Button type="warning" @click="handleRemove(index)" disabled>删除</Button>
          </Col>
        </Row>
      </Form-item>
      <Form-item>
        <Row>
          <Col span="12">
            <Button type="dashed" long @click="handleAdd" icon="plus-round" disabled>新增</Button>
          </Col>
        </Row>
      </Form-item>
    </Form>
...
export default {
  ...
  methods: {
    handleSubmit () {
      this.$http.post('Home/Ticket/update', {id: this.editid, name: this.activity.activity_name, type: this.activity.activity_type, place: this.activity.activity_place, detail: this.activity.textarea}).then((response) => {
    ...
    fetchDate () {
      ...
      this.$http.post('Home/Ticket/read', {id: this.editid}).then((response) => {
      var editdetail = response.body.info;
      this.activity.activity_name = editdetail.ticket.name;
      this.activity.activity_type = editdetail.ticket.type;
      this.activity.activity_place = editdetail.ticket.place;
      this.activity.textarea = editdetail.ticket.detail;
      this.activity.cutofftime = new Date(editdetail.ticket.cutofftime * 1000);
  ...
  created () {
    ...
    // });
    // this.$http.post('Home/Index/index').then((response) => {
    //   this.tickets = response.body.info;
    // });
    this.fetchDate();
  }
